...
FROM mcr.microsoft.com/dotnet/core/sdk:2.2.401-alpine3.9 AS builder-backend

WORKDIR /app/backend
ADD backend/Backend.csproj backend/global.json backend/nuget.config ./
RUN dotnet restore
COPY ./backend ./

WORKDIR /app/backend-tests
COPY backend-tests/BackendTests.csproj backend-tests/global.json backend/nuget.config ./
RUN dotnet restore
COPY ./backend-tests ./
RUN dotnet test
...
WORKDIR /app
COPY --from=builder-backend /app/backend/out .
